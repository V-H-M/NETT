...
#define base3    (816)
#define base4    (624)
#define NUMBER_OF_ENCRYPTIONS (500000)

int st0[Required_Numberof_Element2]={};
int st1[Required_Numberof_Element2]={};
int st2[Required_Numberof_Element2]={};
int st3[Required_Numberof_Element2]={};
int *M0[Required_Numberof_Element1]={};
int *M1[Required_Numberof_Element1]={};
int *M2[Required_Numberof_Element1]={};
int *M3[Required_Numberof_Element1]={};
...
int bot_elems(double *arr, int N, int *bot, int n) {
  /*
     insert into bot[0],...,bot[n-1] the indices of n bigest elements 
     of arr[0],...,arr[N-1]
  */
  int bot_count = 0;
  int i;
...
for (int mm=0;mm<Required_Numberof_Element1;mm++)
{
M0[mm]=&st0[base1+(4096*mm)];
}
for (int mm=0;mm<Required_Numberof_Element1;mm++)
{
M1[mm]=&st1[base2+(4096*mm)];
}
for (int mm=0;mm<Required_Numberof_Element1;mm++)
{
M2[mm]=&st2[base3+(4096*mm)];
}
for (int mm=0;mm<Required_Numberof_Element1;mm++)
{
M3[mm]=&st3[base4+(4096*mm)];
}
...
maccess(M0[mm]);
   ...
   size_t time = rdtsc();
    ...
    totalEncs[2][(int) ciphertext[2]]=totalEncs[2][(int) ciphertext[2]]+delta;
    totalEncs[6][(int) ciphertext[6]]=totalEncs[6][(int) ciphertext[6]]+delta;
    totalEncs[10][(int) ciphertext[10]]=totalEncs[10][(int) ciphertext[10]]+delta;
    totalEncs[14][(int) ciphertext[14]]=totalEncs[14][(int) ciphertext[14]]+delta;
    
      Counter[2][(int) ciphertext[2]]++;
      Counter[6][(int) ciphertext[6]]++;
      Counter[10][(int) ciphertext[10]]++;
      Counter[14][(int) ciphertext[14]]++;
...
maccess(M1[mm]);
   ...
   AES_encrypt(plaintext, ciphertext, &key_struct);
    size_t delta = rdtsc() - time;
    totalEncs[3][(int) ciphertext[3]]=totalEncs[3][(int) ciphertext[3]]+delta;
    totalEncs[7][(int) ciphertext[7]]=totalEncs[7][(int) ciphertext[7]]+delta;
    totalEncs[11][(int) ciphertext[11]]=totalEncs[11][(int) ciphertext[11]]+delta;
    totalEncs[15][(int) ciphertext[15]]=totalEncs[15][(int) ciphertext[15]]+delta;
    
      Counter[3][(int) ciphertext[3]]++;
      Counter[7][(int) ciphertext[7]]++;
      Counter[11][(int) ciphertext[11]]++;
      Counter[15][(int) ciphertext[15]]++;
...
maccess(M2[mm]);
     ...
     AES_encrypt(plaintext, ciphertext, &key_struct);
    size_t delta = rdtsc() - time;
    totalEncs[0][(int) ciphertext[0]]=totalEncs[0][(int) ciphertext[0]]+delta;
    totalEncs[4][(int) ciphertext[4]]=totalEncs[4][(int) ciphertext[4]]+delta;
    totalEncs[8][(int) ciphertext[8]]=totalEncs[8][(int) ciphertext[8]]+delta;
     totalEncs[12][(int) ciphertext[12]]=totalEncs[12][(int) ciphertext[12]]+delta;
   
      Counter[0][(int) ciphertext[0]]++;
      Counter[4][(int) ciphertext[4]]++;
      Counter[8][(int) ciphertext[8]]++;
      Counter[12][(int) ciphertext[12]]++;
     }
...
maccess(M3[mm]);
    ...
    size_t time = rdtsc();
     AES_encrypt(plaintext, ciphertext, &key_struct);
    size_t delta = rdtsc() - time;
    totalEncs[1][(int) ciphertext[1]]=totalEncs[1][(int) ciphertext[1]]+delta;
    totalEncs[5][(int) ciphertext[5]]=totalEncs[5][(int) ciphertext[5]]+delta;
    totalEncs[9][(int) ciphertext[9]]=totalEncs[9][(int) ciphertext[9]]+delta;
    totalEncs[13][(int) ciphertext[13]]=totalEncs[13][(int) ciphertext[13]]+delta;
      Counter[1][(int) ciphertext[1]]++;
      Counter[5][(int) ciphertext[5]]++;
      Counter[9][(int) ciphertext[9]]++;
      Counter[13][(int) ciphertext[13]]++;
  ...
  for (int i=0; i<16; i++) {
    for (int j=0; j<256; j++) {
      average[i][j] = (double)totalEncs[i][j] / Counter[i][j]; }
  }

  int botIndices[16][16];
  // get the values of bigest averages
  for (int i=0; i<16; i++) {
    bot_elems(average[i], 256, botIndices[i], 16);
  }

  for (int i=0; i<16; i++) {
    // loop through ciphertext bytes with lowest averages
    for (int j=0; j<16; j++) {
